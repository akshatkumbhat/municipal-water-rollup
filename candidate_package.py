...
def render_ic_summary(
    ...
    add("")
    add("### What the evidence actually supports")
    add("")
    add("`blueprint` is the most common tag in this document, and it is the one most")
    add("easily misread. It means *asserted in `PROJECT_BLUEPRINT.md`* — a document")
    add("written by the same author, for this exercise. It is a statement of where a")
    add("number came from, **not evidence that the number is right.** No tag in the")
    add("table above carries external validation.")
    add("")
    bench = assumption_benchmark_table(SCENARIOS["base"])
    total = len(bench)
    strong = int(bench["Tier"].str.startswith(("[A]", "[B]")).sum())
    industry = int(bench["Tier"].str.startswith("[C]").sum())
    indicative = int(bench["Tier"].str.startswith("[D]").sum())
    unbenchmarked = int(bench["Tier"].eq("none").sum())
    add(f"Of the {total} material model inputs checked against published sources in")
    add(f"`04_reference/assumption_benchmarks.csv`: **{strong} are supported by")
    add(f"peer-reviewed or government evidence.** {industry} rest on industry-research")
    add(f"`[C]` sources, {indicative} on practitioner `[D]` material that is marked")
    add(f"indicative only and is not load-bearing, and {unbenchmarked} had no usable")
    add("published figure at all.")
    add("")
    add("The market thesis is the part that *is* evidenced — municipal water")
    add("infrastructure need is documented at `[B]` in `RESEARCH_BENCHMARKS.md`. The")
    add("returns are not. The gap between those two statements is the honest position")
    add("of this case.")
    add("")
    outside = bench[~bench["Verdict"].str.startswith("Inside")]
    add(f"{len(outside)} of the {total} sit outside the cited range or have no range")
    add("at all:")
    add("")
    add("| Input | Model | Verdict | Tier |")
    add("|---|---|---|---|")
    for _, row in outside.iterrows():
        add(f"| {row['Assumption']} | {row['Model value']} | {row['Verdict']} | {row['Tier']} |")
    add("")
    add("**Three of those cut in favour of the returns**: the platform margin sits")
    add("above the cited operating band, and both the add-on entry multiple and the")
    add("interest rate sit below theirs — richer EBITDA, cheaper acquisitions, cheaper")
    add("debt. One cuts against: the exit mark is held below the range deliberately.")
    add("A reviewer should press the first three before crediting the last.")
    add("")
    add("---")
    add("")
    ...
    add("")
    add("`integration-failure` holds every platform assumption at base and applies")
    add("only the damaged acquisition schedule, costing")
    add(f"{_turns(br['gross_moic'] - itf['gross_moic'])} of MOIC against the base case.")
    add("**That is the whole of what this pair supports.**")
    add("")
    add("> **Retraction.** An earlier version of this section subtracted")
    add("> `integration-failure` from `severe-downside` and concluded that integration")
    add("> risk was secondary to entry-price and margin risk. That was invalid. The two")
    add("> cases do not differ by integration alone: `severe-downside` additionally")
    add("> degrades synergy capture and its first-year realization, which")
    ...
    add("")
    add("The same separation applies to the tuck-ins. A stressed add-on that misses")
    add("its margin case was previously bought at the lower EBITDA too, so execution")
    add("failure partly paid for itself; the two stressed tuck-ins were acquired")
    add("**$539k cheaper precisely because they underperformed**. Their prices are now")
    add("struck on underwritten margins, giving effective entry multiples of 4.85x and")
    add("4.20x against a 3.5x bid — the tuck-in analogue of the platform's 8.5x.")
    add("")
    add("Two consequences the committee should note. **The previous severe case")
    add("understated the loss**, because both stresses were quietly repricing what was")
    add("bought. And **margin is now the second-largest driver**, behind only organic")
    add("growth — the opposite of what the contaminated table implied — while")
    add("integration rises above exit valuation once its repricing offset is removed.")
